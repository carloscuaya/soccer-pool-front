...
function CreateTournament() {
    ...
    const [matchesNumber, setMatchesNumber] = useState<number | ''>('')
    ...
    }, [navigate, setTopNavActions, username])

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!name || matchesNumber === '') return
        setLoading(true)
        try {
            await createTournament({ name, matches_number: matchesNumber as number, status })
            sileo.success({ title: t('createTournament.success') })
            navigate('/tournaments-admin')
        } catch {
            sileo.error({ title: t('createTournament.error') })
        } finally {
            setLoading(false)
        }
    }
